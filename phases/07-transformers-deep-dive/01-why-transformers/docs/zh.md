...
梯度消失意味50词元前的信息已经压缩过50个非线性。门控循环单元(LSTM, GRU)减轻了这一问题但从未彻底消除。长程依赖——"去年夏天我去京都的飞机上读的那本书是……"——常规失败。
...
**归纳偏置转变。**RNN假设局部和近性。Transformer不假设——每对都是候选注意力。这就是为何transformer需要更多数据训练好但一旦有数据scaling更远。Chinchilla(2022)形式化:给定足够词元,等参数计数transformer总是胜RNN。
...
| 归纳偏置 | "架构中假设" | 关于数据看起来像什么prior;CNN假设平移不变,RNN假设近性。 |
