...
全注意力序列长`O(N²)`内存和`O(N²)`计算。对128K上下文Llama 3 70B每层16亿注意力入口,乘80层。Flash注意力机制(课程12)藏`O(N²)`激活内存但不改算术成本——每词元仍attend每其他词元。

三类变体改注意力矩阵拓扑本身:

1. **滑动窗口注意力(SWA)。**每词元attend固定窗口邻居,非全前缀。内存和计算降到`O(N · W)`,W是窗口。Gemma 2/3、Mistral 7B首层、Phi-3-Long。
...
每位置`i`query仅attend`[i - W, i]`位置(因果SWA)或`[i - W/2, i + W/2]`(双向)。窗口外词元得分数矩阵`-inf`。
...
见`code/main.py`。实现因果掩码比较器玩具序列并排示全、SWA、局部+跨步、差分注意力。
...
两注意力pass,配学习混合系数减。代码比单vs差分注意力汇热图并观汇坍。
...
- **纯粹全注意力**——每层到~16K上下文,或检索质量 paramount。
...
见`outputs/skill-attention-variant-picker.md`。技能给定目标上下文长、检索需求和训练/推理计算profile为新模型选注意力拓扑。
...
| 术语 | 人们怎么说 | 实际含义 |
|------|------------|----------|
| 滑动窗口注意力(SWA) | "局部注意力" | 每query attend其最后W词元;KV cache缩到O(W)。 |
