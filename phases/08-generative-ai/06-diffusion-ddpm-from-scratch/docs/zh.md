...
> Ho, Jain, Abbeel(2020)给领域一个它不能quit的配方。一千小步用噪声破坏数据。训一个神经网络预测噪声。推理时反向过程。今天每个主流图像、视频、3D和音乐模型跑在此循环上,可能配流匹配或一致性技巧顶。
...
- **调度重要。**线性`β`是DDPM默认但余弦调度(Nichol & Dhariwal, 2021)同算给更好FID。质量 plateau 时换调度。
...
DDPM论文跑T=1000反向步。无人生产发那。每真实推理栈选三策略之一——每清晰映射生产框定"延迟从何来":
