...
> 每个生成模型排行榜都引用FID、CLIP分数和人类偏好竞技场胜率。每个指标都有坚定的研究员可以博弈的失败模式。如不了解失败模式，就无法区分真实改进和刷榜结果。
...
- **依赖Inception。**Inception-v3训练于ImageNet。远离ImageNet域(人脸、艺术、文档图像)产生无意义FID。改用领域特定的特征提取器。
...
- **N=1000 FID。**经验表明N<10k时FID不可靠。报告低N FID的论文是在刷榜。
