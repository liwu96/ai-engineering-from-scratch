...
两者受生成顺序问题。像素和词元排2D网格,但AR模型需1D光栅顺序访。早角像素无图像终成何概念。生成质量缩放比GPT-on-text差且匹配计算从未达扩散模型质量。
...
它们是问题不同轴。两者产可处理条件分布。经验VAR推理更快(更少pass,尺度内全并行)并类条件ImageNet匹或胜DiT。文本条件VAR(VARclip, HART)是活跃研究方向。
