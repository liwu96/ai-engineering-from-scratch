...
> 正确工具当模型不可辨何时用时静默失败。命名、描述、参数形于StableToolBench和MCPToolBench++等bench驱动工具择准确度10至20百分点波动。本课命名分离模型可靠择的工具和模型误发的工具的设计规。
...
Composio 2025野外指南测内bench仅重命名和重写描述驱动10至20百分点准确度波动。Anthropic Agent SDK文档声类似。Databricks agent模式文档更:50工具带歧义描述注册择准确度跌至62%;描述重写后同注册至89%。
