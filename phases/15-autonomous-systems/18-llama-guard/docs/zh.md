...
**表情符号走私**（Huang等人，arXiv:2504.11168）：在禁止请求的字符间插入不可打印或视觉上相似的emoji。分词器以与分类器预期不同的方式合并它们。六个知名guard系统上100% ASR。
...
**NeMo Guard Detect**：Huang等人论文中越狱基准上72.54% ASR。这是精心构造的攻击；随意越狱的成功率要低得多，但天花板显然不是"零。"
...
5. NeMo Guard Detect在越狱基准上的72.54% ASR在对抗性构造下测量。设计在随意（非对抗性）用户分布下测量分类器ASR的评估协议。你会预期什么数字，为什么该数字单独重要？
